...
export class ProvablyFairService implements IProvablyFairService {
  ...
  calculateCrashPoint(
    ...
    // crashPoint = 99 / (1 - hashValue) * (1 - houseEdge)
    // This ensures the house has a 3% edge on average
    const crashPointValue =
      (99 / (1 - hashValue)) * (1 - this.houseEdge);

    // Round to 2 decimal places
    const rounded = Math.round(crashPointValue * 100) / 100;

    // Ensure minimum crash point of 1.00x
    const finalValue = Math.max(rounded, 1.0);
